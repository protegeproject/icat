...
public class NotesGrid extends GridPanel {
	...
	*/
    ...
    protected void onReplyButton(NotesData note) {
        ...
        final EntityData noteEntity = note.getEntity();
        final String entityName = (noteEntity == null ? "" : noteEntity.getName());
    ...
    private void addReplyToStore(NotesData note, int indent) {
        if(note.getReplies() != null){
            ...
            while (it.hasNext()) {
                ...
                final EntityData noteEntity = n.getEntity();
                final String entityName = (noteEntity == null ? "" : noteEntity.getName());
    ...
    class GetNotes extends AbstractAsyncHandler<List<NotesData>> {
        ...
        public void handleSuccess(List<NotesData> notes) {
            ...
            while (iterator.hasNext()) {
                ...
                final EntityData noteEntity = note.getEntity();
                final String entityName = (noteEntity == null ? "" : noteEntity.getName());
                Record record = recordDef.createRecord(new Object[] { entityName, subject,
                        note.getAuthor(), time, note.getBody(), note.getType(), entityName, getRepliesCount(note) });
                store.add(record);
