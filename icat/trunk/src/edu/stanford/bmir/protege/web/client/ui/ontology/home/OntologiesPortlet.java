...
public class OntologiesPortlet extends AbstractEntityPortlet {
	...
	private static String LINK_TO_PROTEGE_JNLP = "<a href=\"" +
				GWT.getModuleBaseURL() + "collabProtege_OWL.jnlp" + "\">here</a>";
	...
	public void intialize() {
		...
		setTitle("Ontologies");		
		createGrid();// TODO Auto-generated method stub
		add(ontologiesGrid);
		addJNLPPanel();		
	}

	private void addJNLPPanel() {
		Panel p = new Panel();	
		Label label =  new Label();
		//FIXME: the <br> is a hack - fix in css
		label.setHtml("<html><br><br><span class=\"jnlpPanel\">To open the same copy of an ontology" +
				" in the Prot&eacute;g&eacute rich client, click "
			+ LINK_TO_PROTEGE_JNLP + " .</span><br><br></html>");
		p.add(label);
		add(p);
	}
	...
	protected void createColumns() {
		...
		Renderer nameColRenderer = new Renderer() {
			public String render(Object value, CellMetadata cellMetadata,
				...
				return new String("<a href=\"#" + value.toString() + "\">"
						+ value.toString() + "</a>");
			}
		};
		...
		actionCol.setDataIndex("action");
		actionCol.setResizable(true);
		...
		*/
		/*
		Renderer actionColRenderer = new Renderer() {
			public String render(Object value, CellMetadata cellMetadata, 
					Record record, int rowIndex, int colNum, Store store) {				
	...
	public ProjectData getOntologyData(String projectName) {
		return (ProjectData) ontologies.get(projectName);
	}
	...
	public void refreshProjectList() {
		store.removeAll();
		AdminServiceManager.getInstance().getProjects(project == null ? null : project.getUserName(), new GetProjectsHandler());	
	}
	...
	class GetProjectsHandler extends AbstractAsyncHandler<ArrayList<ProjectData>> {

		public void handleFailure(Throwable caught) {
			GWT.log("RPC error getting ontologies from server", caught);			
		}

		public void handleSuccess(ArrayList<ProjectData> result) {
			ArrayList<ProjectData> results = (ArrayList<ProjectData>) result;
			Iterator<ProjectData> i = results.iterator();
			while (i.hasNext()) {
				ProjectData data = (ProjectData) i.next();
	...
	class RefreshMetaProjectHandler extends AbstractAsyncHandler {
		...
		public void handleSuccess(Object result) {
			com.google.gwt.user.client.Window.alert("Metaproject information refreshed successfully.\n" +
