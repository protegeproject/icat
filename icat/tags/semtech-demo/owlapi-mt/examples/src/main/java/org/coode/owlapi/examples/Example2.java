...
public class Example2 {
    ...
    public static void main(String[] args) {
        try {
            ...
            List<OWLOntologyChange> lst= new ArrayList<OWLOntologyChange>(2);
            
            lst.add(addAxiom);
            
            // Now we create the individual
    		OWLNamedIndividual individual = factory.getOWLNamedIndividual(IRI.create(ontologyIRI + "#AInd")); 
    		// We can add a declaration axiom to the ontology, that essentially adds the instance to the signature of
    		// our ontology.
    		lst.add(new AddAxiom(ontology, factory.getOWLDeclarationAxiom(individual)));

    		OWLClass type = factory.getOWLClass(IRI.create(ontologyIRI + "#A"));

    		// We now add the axiom to the ontology, so that the ontology states that
    		// A is a subclass of B.  To do this we create an AddAxiom change object.
    		lst.add( new AddAxiom(ontology, factory.getOWLClassAssertionAxiom(type, individual)));
    		
    		manager.applyChanges(lst);
            ...
            System.out.print("Asserted superclasses of " + clsA + ":  ");
            for (OWLClassExpression desc : superClasses) {
                System.out.println(desc);
            }
            
            for (OWLNamedIndividual ind : ontology.getIndividualsInSignature()) {
                System.out.println("Individuals: " + ind);
            }
            
            for (OWLEntity ent : ontology.getEntitiesInSignature(IRI.create(ontologyIRI + "#AInd"), true)) {
                System.out.println("Entities: " + ent);
            }
