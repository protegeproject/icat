    ...
    public static Comparator<OWLObject> getOWLObjectComparator() {
        return new Comparator<OWLObject>() {
            ...
            public int compare(OWLObject o1, OWLObject o2) {
                return Util.getBrowserText(o1, null).compareTo(Util.getBrowserText(o2, null));
            }
    ...
    public static EntityData createEntityData(OWLObject object) {
        ...
        if (OWLEntity.class.isAssignableFrom(object.getClass())) {
            ...
            if (OWLProperty.class.isAssignableFrom(entity.getClass()) || OWLAnnotationProperty.class.isAssignableFrom(entity.getClass())) {
                ...
                entityProp = new PropertyEntityData(entity.getIRI().toURI().toString(), getBrowserText(entity, null));
...
//			}
            ...
            return new EntityData(entity.getIRI().toURI().toString(), getBrowserText(entity, null));
            //						(computeAnnotations) ? ChAOUtil.hasAnnotations(objFrame) : false);
        }
        if (OWLLiteral.class.isAssignableFrom(object.getClass())) {
            OWLLiteral literal = (OWLLiteral) object;
            PropertyEntityData entityProp = new PropertyEntityData(literal.getLiteral(), getBrowserText(object, null));
    ...
    public static String fixLanguageTagBug(String str) {
    ...
    public static String fixTagBug(String str) {
        int length = str.length();
        if (str.lastIndexOf("\"@") == length - 2) {
            return str.substring(1, length - 2);
        }
        if (str.lastIndexOf("\"@") == length - 4) {
            return str.substring(1, length - 4);
        }
        return str;
    }
    ...
    public static PropertyEntityData createPropertyEntityData(OWLOntology onto, OWLEntity property, boolean computeAnnotations) {
        ...
        PropertyEntityData ped = new PropertyEntityData(property.toStringID(), getBrowserText(property, onto));
...
//	}
    ...
    static public String getBrowserText(OWLObject obj, OWLOntology owlOntology) {
        Expression exp = new Expression();

        exp.getBrowserText(obj, owlOntology);
...
//		exp.append("(");
        getBrowserText(some.getProperty(), null);
        exp.append(" some ");
        getBrowserText(some.getFiller(), null);
//		exp.append(")");
    }

    public void getBrowserText(OWLObject obj, OWLOntology owlOntologyManager) {
        if (obj instanceof OWLNamedObject) {
            getBrowserText(((OWLNamedObject) obj).getIRI(), obj, owlOntologyManager);
        }
        obj.accept(this);
    }

    public void getBrowserText(IRI uri, OWLObject obj, OWLOntology owlOntologyManager) {
        ...
        //TODO: the following sections deal with the distinctive way that the AEO ontology wants things rendered.
        // We need to figure out a more generic way to make the browser text generation pluggable, probably by reimplementing all of this logic
        // in a completely different fashion (especially the interfaces, which are now a bit nasty). 
        if (uri.toString().startsWith("http://purl") && owlOntologyManager != null && OWLClassImpl.class.isAssignableFrom(obj.getClass())) {
            final Set<OWLAnnotation> set = ((OWLClassImpl) obj).getAnnotations(owlOntologyManager);
            final String s = getLabelAnnotationValue(obj, owlOntologyManager, set);
            exp.append(s);
            return;
        }
        if (uri.toString().startsWith("http://purl") && owlOntologyManager != null && OWLAnnotationProperty.class.isAssignableFrom(obj.getClass())) {
            final Set<OWLAnnotation> set = ((OWLAnnotationProperty) obj).getAnnotations(owlOntologyManager);
            final String s = "rdfs:label";
            exp.append(s);
            return;
        }
    ...
    private String getLabelAnnotationValue(OWLObject obj, OWLOntology owlOntologyManager, Set<OWLAnnotation> set) {
        ...
        for (OWLAnnotation owlAnnotation : set) {
            ...
            if (owlAnnotation.getProperty().getIRI().toString().equals("http://www.w3.org/2000/01/rdf-schema#label")) {
                map.put(owlAnnotation.getProperty(), Arrays.asList("en", "pt", "", "fr", " ", "  ", "de", null));
                properties.add(owlAnnotation.getProperty());
            }
        }
        final AnnotationValueShortFormProvider shortFormProvider = new AnnotationValueShortFormProvider(properties, map, owlOntologyManager.getOWLOntologyManager());
        final String s = shortFormProvider.getShortForm((OWLEntity) obj);
        return s;
    }
}
