...
public class OpenIdUtil {
    ...
    public void displayUsersOpenIdList(OpenIdData openIdData, final FlexTable editProfileTable,
        ...
        editProfileTable.setWidget(6, 0, openIdListDisclosure);
        editProfileTable.getFlexCellFormatter().setColSpan(6, 0, 2);
    ...
    public void openProviderForOpenIdAuth(final int provId, boolean isLoginWithHttps) {
        ...
        openNewWindow(openidUrl, "500", "400", "1");
    }
        ...
        final Timer checkSessionTimer = new Timer() {
            ...
            public void run() {
                ...
                if (createUserRes != null
                    ...
                    OpenIdServiceManager.getInstance().checkIfUserCreatedToAssocOpenId(new AsyncCallback<String>() {
                            ...
                            new AsyncCallback<String>() {
                                ...
                                public void onSuccess(String userName) {
                                    if (userName != null) {
                                        ...
                                        timer.cancel();
                                        MessageBox
                                                .alert("Your WebProtege account was successfully associated with the Open Id");
                                    }
        ...
        if (newUserPassword.contentEquals(newUserPassword2)) {
            ...
            AdminServiceManager.getInstance().getNewSalt(new AsyncCallback<String>() {
                ...
                public void onSuccess(String salt) {
                    ...
                    OpenIdServiceManager.getInstance().regUserToAssocOpenIdWithEncrption(userName, saltedHashedPass,
                            emailId, new AsyncCallback<UserData>() {

                                public void onSuccess(UserData userData) {
                                    UserRegData userRegData = (UserRegData) userData;
                                    win.getEl().unmask();
                                    String result = userRegData.getResult();
                                    if (userRegData == null || result == null
                                            || result.equals(OpenIdConstants.REGISTER_USER_ERROR)) {
                                        MessageBox
                                                .alert("New user registration could not be completed. Please try again.");
                                    } else {
                                        if (result != null
                                                && userRegData.getResult().equals(OpenIdConstants.USER_ALREADY_EXISTS)) {
                                            MessageBox.alert("User '" + userName + "' already exists.");
                                        } else if (result != null
                                                && result.equals(OpenIdConstants.REGISTER_USER_SUCCESS)) {
                                            win.close();
                                            GlobalSettings.getGlobalSettings().getGlobalSession().setUserName(
                                                    userData.getName());
                                            MessageBox
                                                    .alert("New user created successfully and associated with your Open Id.");
                                        }
                                ...
                                public void onFailure(Throwable caught) {
                                    ...
                                    win.getEl().unmask();
                                    MessageBox
                                            .alert("There was an error at creating the new user. Please try again later.");
    ...
    class createNewUserToAssocOpenIdHandler extends AbstractAsyncHandler<UserData> {
        ...
        public void handleSuccess(UserData userData) {
            ...
            expireDate.setTime(nowLong);
            String result = userRegData.getResult();
            if (userRegData == null || result == null || result.equals(OpenIdConstants.REGISTER_USER_ERROR)) {
                MessageBox.alert("New user registration could not be completed. Please try again.");
            } else {
                if (result != null && userRegData.getResult().equals(OpenIdConstants.USER_ALREADY_EXISTS)) {
                    MessageBox.alert("User '" + userName + "' already exists.");
                } else if (result != null && result.equals(OpenIdConstants.REGISTER_USER_SUCCESS)) {
                    win.close();
                    Cookies.setCookie(AuthenticationConstants.CREATE_USER_TO_ASSOC_OPENID_RESULT,
                            AuthenticationConstants.CREATE_USER_TO_ASSOC_OPENID_SUCCESS, expireDate);
                    closeWindow();
                }
