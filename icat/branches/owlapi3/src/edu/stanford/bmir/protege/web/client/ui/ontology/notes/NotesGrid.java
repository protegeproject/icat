	...
	*/
    ...
    protected void onReplyButton(NotesData note) {
        ...
        Record plant = recordDef.createRecord(new Object[] { "newAnnId",
                ...
                note.getCreationDate(), note.getBody(), note.getType(), note.getEntity().getName(), getRepliesCount(note) });
    ...
    protected void onPostButton(NotesData note) {
        ...
        Record plant = recordDef.createRecord(new Object[] { "newAnnId", subject,
                ...
                note.getBody(), note.getType(), note.getEntity().getName(), getRepliesCount(note) });
    ...
    private void addReplyToStore(NotesData note, int indent) {
        if(note.getReplies() != null){
            ...
            while (it.hasNext()) {
                ...
                Record record = recordDef.createRecord(new Object[] { n.getEntity().getName(), subject, n.getAuthor(),
                        time, n.getBody(), n.getType(), n.getEntity().getName(), getRepliesCount(n) });
                store.add(record);
    ...
    class GetNotes extends AbstractAsyncHandler<List<NotesData>> {
        ...
        public void handleSuccess(List<NotesData> notes) {
            ...
            while (iterator.hasNext()) {
                ...
                Record record = recordDef.createRecord(new Object[] { note.getEntity().getName(), subject,
                        note.getAuthor(), time, note.getBody(), note.getType(), note.getEntity().getName(), getRepliesCount(note) });
                store.add(record);
