...
public class BioPortalSearchComponent extends GridPanel {
    ...
    private enum DefaultSearchStringType {
        None("@none@"), Entity("@entity@"), Property("@property@");

        private String stringValue;

        DefaultSearchStringType(String label) {
    ...
    protected ToolbarButton createDNFRefButton;
    ...
    private void onImportReference(Record record) {
        ...
        OntologyServiceManager.getInstance().createExternalReference(
                ...
                _currentEntity.getName(),
                bpRefData,
                GlobalSettings.getGlobalSettings().getUserName(),
                UIUtil.getAppliedToTransactionString("Imported reference for " + getEntity().getBrowserText()
                        + " Reference: " + bpRefData.getPreferredName() + ", code: " + bpRefData.getConceptId(),
                        getEntity().getName()), getImportBioPortalConceptHandler());
    }
    ...
    public void setEntity(EntityData newEntity, boolean refreshUI) {
        ...
        } else if (defSearchString.equals(DefaultSearchStringType.None.toString())) {
            searchStringTextField.setValue("");
        } else if (defSearchString.equals(DefaultSearchStringType.Entity.toString())) {
            searchStringTextField.setValue(_currentEntity.getBrowserText());
        } else if (defSearchString.startsWith(DefaultSearchStringType.Property.toString())) {
    ...
    class CreateManualreferenceHandler extends AbstractAsyncHandler<EntityData> {
        ...
        public void handleFailure(Throwable caught) {
            getEl().unmask();
            GWT.log("Could not create manual reference for " + _currentEntity, null);
    ...
    class CreateDNFConceptHandler extends AbstractAsyncHandler<EntityData> {
        ...
        public void handleFailure(Throwable caught) {
            getEl().unmask();
            GWT.log("Could not create DNF reference for " + _currentEntity, null);
            MessageBox.alert("Reference creation failed!");
        }
