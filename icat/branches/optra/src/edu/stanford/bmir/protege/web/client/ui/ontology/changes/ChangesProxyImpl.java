...
import edu.stanford.bmir.protege.web.client.ui.ontology.changes.filter.ChangesFilterData;
...
public class ChangesProxyImpl extends GWTProxy {
    ...
    public void load(int start, int limit, String sort, String dir, final JavaScriptObject o, UrlParam[] baseParams) {
        ...
        if (ChangesFilterData.isFilterDataCurrent) {
            
            ChangesFilterData.isFilterDataCurrent = false;
            if (ChangesFilterData.filterType != null && ChangesFilterData.filterType.equals(ChangesFilterData.filterTypeList[0])) {
                ChAOServiceManager.getInstance().getChangesByAuthor(projectName, entityName, ChangesFilterData.filterInputText,
                        start, limit, sort, dir, new ChangesHandler());
            } else if (ChangesFilterData.filterType != null
                    && ChangesFilterData.filterType.equals(ChangesFilterData.filterTypeList[1])) {
                ChAOServiceManager.getInstance().getChangesByDate(projectName, entityName, ChangesFilterData.fromDate, ChangesFilterData.toDate, start, limit,
                        sort, dir, new ChangesHandler());
            } else if (ChangesFilterData.filterType != null
                    && ChangesFilterData.filterType.equals(ChangesFilterData.filterTypeList[2])) {
                ChAOServiceManager.getInstance().getChangesByDescription(projectName, entityName,
                        ChangesFilterData.filterInputText, start, limit, sort, dir, new ChangesHandler());
            }

        } else if (entityName != null) {
            ChAOServiceManager.getInstance().getChanges(projectName, entityName, start, limit, sort, dir, new ChangesHandler());
