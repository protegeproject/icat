...
public class LoginUtil {
    ...
    public void login(final boolean isLoginWithHttps) {
        ...
        if (isLoginWithHttps) {
            OpenIdUtil
                    .detectPopup("Your pop-up blocker has hidden the login window. Please disable the pop-up blocker and click on 'Sign In' again.");
        }
        ...
        if (ClientApplicationPropertiesCache.getWebProtegeAuthenticateWithOpenId()) {
            ...
            addFacebookButton(win, loginTable, isLoginWithHttps, 5);
    ...
    private String getHintHtml() {
        return UIUtil.getHelpImageHtml("http://protegewiki.stanford.edu/wiki/WebProtegeOpenId",
    ...
    public void checkIfOpenIdInSessionForLogin(final String providerType) {
            ...
            public void onSuccess(UserData userData) {
                if (userData != null) {
                    if (userData.getName() != null) {
                        ...
                        enquireOpenIdAssociation(providerType);
                    }
                }
            }
        ...
        MessageBox.confirm("Log out", "Are you sure you want to log out?", new MessageBox.ConfirmCallback() {
            public void execute(String btnID) {
                if (btnID.equalsIgnoreCase("yes")) {
                    ...
                    ShareOntologyAccessEventManager.getShareOntologyAccessManager().notifyToUpdateShareLink(false);
                        ...
                        if (loggedInMethod != null) {
                            ...
                            if (loggedInMethod.equals(AuthenticationConstants.LOGIN_METHOD_WEBPROTEGE_ACCOUNT)) {
                                checkUserNameInSession();
                                    ...
                                    || loggedInMethod.equals(AuthenticationConstants.LOGIN_METHOD_FACEBOOK_ACCOUNT)) {
                                checkIfOpenIdInSessionForLogin(AuthenticationConstants.ACCOUNT_TYPE_OPENID);
                            }
    ...
    public void changePassword(final String userName, boolean isLoginWithHttps) {
        ...
        if (isLoginWithHttps) {
            ...
            changePasswordButton.addListener(new ChangePasswordWithEncryptionListener(win, newConfirmPassword, userName,
                    oldPasswordField, newPasswordField));
        }
                ...
                if (event.getNativeKeyCode() == KeyCodes.KEY_ENTER) {
                    if (newUserEmailID.getText().trim().equals("") || newUserPassword.getText().trim().equals("")
                        ...
                        if (isLoginWithHttps) {
                            ...
                            createNewUserViaEncryption(newUserID.getText(), newUserPassword, confirmPassword, newUserEmailID
                                    .getText(), win);
                        }
                    }
                }
    ...
    private void appendAddMoreUsersLink(HTML addMoreUserHTML, final boolean isLoginWithHttps) {
        String addMoreUserLinkImage = "<img src='" + GWT.getHostPageBaseURL() + "images/shareaccess/user_add.png'/>";
        addMoreUserHTML.setHTML("<br><a href='javascript:;'>" + addMoreUserLinkImage
                + "<b><span style='font-size:100%; text-decoration:underline;'>" + " Add more users" + "</span></b></a><br><br>");

        addMoreUserHTML.addClickHandler(new ClickHandler() {

            public void onClick(ClickEvent event) {

                if (isLoginWithHttps) {
                    String httpsPort = ClientApplicationPropertiesCache.getApplicationHttpsPort();
                    String authUrl = getAuthenticateWindowUrl(AuthenticationConstants.AUTHEN_TYPE_CREATE_USER, httpsPort);
                    openNewWindow(authUrl, "440", "260", "0");
                } else {
                    createNewUser(isLoginWithHttps);
                }
            }
        });
    }
    ...
    public void createNewUserToAssociateOpenId(final Boolean isLoginWithHttps) {
        ...
        Label label = new Label(
                "Welcome! Please enter your name, password and email to create a new user account that will be associated with OpenId.");
        label.setStyleName("login-welcome-msg");
                ...
                AuthenticateServiceManager.getInstance().validateUser(userName, oldPasswordField.getText(),
                        new AbstractAsyncHandler<UserData>() {
                            ...
                            public void handleSuccess(UserData userData) {
                                win.getEl().unmask();
                                if (userData != null) {
                                    AuthenticateServiceManager.getInstance().changePassword(userName, newPasswordField.getText(),
                                            new ChangePasswordHandler(win));
                                } else {
            ...
            if (newPasswordField.getText().trim().equals(newConfirmPassword.getText().trim())) {
                ...
                AdminServiceManager.getInstance().getUserSaltAndChallenge(userName, new AsyncCallback<LoginChallengeData>() {
                    ...
                    public void onSuccess(LoginChallengeData result) {
                        if (result != null) {
                            validateEncryptPassAndChangePass(result, win, newConfirmPassword, userName, oldPasswordField,
                                    newPasswordField);
                        } else {
                            MessageBox.alert("Invalid password. Please try again.");
                            oldPasswordField.setValue("");
    ...
    class ChangePasswordHandler extends AbstractAsyncHandler<Void> {
        ...
        public void handleSuccess(Void result) {
            ...
            Cookies.setCookie(AuthenticationConstants.CHANGE_PASSWORD_RESULT, AuthenticationConstants.CHANGE_PASSWORD_SUCCESS,
                    expireDate);
            closeWindow();
        }
    }
    ...
    public void enquireOpenIdAssociation(String providerType) {
        if (providerType == null) {
            providerType = AuthenticationConstants.ACCOUNT_TYPE_FACEBOOK;
        }
        MessageBox
                .confirm(
                        "Protege account",
                        "Welcome! The first time you log in with your "
                                + providerType
                                + " we need to associate it with a WebProtege account. <br /> Do you have a current WebProtege account?",
                        new ConfirmCallback() {
                            ...
                            AdminServiceManager.getInstance().allowsCreateUsers(new AsyncCallback<Boolean>() {
                                public void onFailure(Throwable caught) {
                                    ...
                                    MessageBox.alert("Error",
                                            "There was an error finding out if the server allows user creation.");
                                }
                                ...
                                public void onSuccess(Boolean result) {
                                    if (result) {
                                        ...
                                        MessageBox
                                                .alert("The creation of new user accounts and linking the user accounts to OpenId is disabled."
                                                        + "<br/><br/>"
     ...
     * 
     * @param isLoginWithHttps
     *            If true then the login popup is opened in a new browser popup
     */
    public void loginToAssociateOpenId(final Boolean isLoginWithHttps) {
        ...
        label.setText("The first time you log in with your Open Id, we need to associate your Open Id with a user account. "
                + "Please enter the username and password of your account below:");
        label.setStyleName("login-welcome-msg");
    ...
    public native void openNewWindow(String url, String w, String h, String scrollbar)/*-{
		...
		var paramz = 'status=0,toolbar=0,menubar=0,location=0,resizable=0,scrollbars='
				+ scrollbar + ',width=' + w + ',height=' + h + '';
    ...
    /**
     * Adds facebook login/logout button on login window.
     */
    private void addFacebookButton(final Window win, final FlexTable loginTable, final boolean isLoginWithHttps, final int row) {

        final String fbApiKey = ClientApplicationPropertiesCache.getFacebookAPIKey();
        FacebookLoginUtil.setLoginWithHttps(isLoginWithHttps);
        FacebookLoginUtil.setLoginWindow(win);
        FacebookLoginUtil fLoginUtil = new FacebookLoginUtil();
        fLoginUtil.addFacebookConnect(loginTable, row);
    }
                ...
                if (userData != null) {
                    AdminServiceManager.getInstance().getNewSalt(new AsyncCallback<String>() {
                        ...
                        public void onSuccess(String salt) {
                            ...
                            AdminServiceManager.getInstance().changePasswordEncrypted(userName, saltedHashedNewPass, salt,
    ...
    protected void associateCurrentWebProtegeAccount() {
        ...
        OpenIdServiceManager.getInstance().clearAuthUserToAssocOpenIdSessData(new AsyncCallback<Void>() {
            ...
            public void onSuccess(Void result) {
                ...
                String authUrl = loginUtil.getAuthenticateWindowUrl(AuthenticationConstants.AUTHEN_TYPE_LOGIN_TO_ASSOC_OPEN_ID,
                        httsPort);
     ...
     * @param passwordField
     */
    protected void callSignAndAssocOpenId(final Boolean isLoginWithHttps, final Window win, final TextBox userNameField,
            final TextBox passwordField) {
