...
import edu.stanford.bmir.protege.web.client.ui.ontology.changes.filter.FilterUtil;
...
public class ChangesPortlet extends AbstractEntityPortlet {
    ...
    public void initialize() {
        ...
        filterUtil.addFilterWidgetToToolbar(getTopToolbar(),store,changesGrid);
    }
    ...
    class GetChangesHandler extends AbstractAsyncHandler<Collection<ChangeData>> {

        @Override
        public void handleFailure(Throwable caught) {
            GWT.log("RPC error getting changes for the " + project.getProjectName() + "ontology", caught);
        }
        ...
        public void handleSuccess(Collection<ChangeData> result) {
            for (ChangeData data : result) {
                 ...
                 * an empty string.
                 */
                Record record = recordDef.createRecord(new Object[] { data.getDescription(), data.getAuthor(),
                        data.getTimestamp(), "" });
                store.add(record);
            }
