...
public class OpenIdServiceImpl extends RemoteServiceServlet implements OpenIdService {
    ...
    public UserData isFacebookProfileInSessForAddNewOpenId() {
        UserData userData = null;
        try {
            HttpServletRequest request = this.getThreadLocalRequest();
            HttpSession session = request.getSession();
            String openIdUrl = (String) session.getAttribute(OpenIdConstants.HTTPSESSION_OPENID_URL);
            String loginMethod = (String) session.getAttribute(AuthenticationConstants.LOGIN_METHOD);

            if (openIdUrl != null && loginMethod != null && loginMethod.equals(AuthenticationConstants.ADD_NEW_OPENID_FACEBOOK_ACCOUNT) ) {
                userData = Protege3ProjectManager.getProjectManager().getMetaProjectManager()
                        .getUserAssociatedWithOpenId(openIdUrl);
                if (userData != null) {
                    if (userData.getName() != null) {
                        session.setAttribute(OpenIdConstants.HTTPSESSION_OPENID_URL, null);
