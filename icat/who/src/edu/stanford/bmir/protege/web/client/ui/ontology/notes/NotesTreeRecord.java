...
public class NotesTreeRecord {
    ...
    private void archiveNote(final boolean archive){
        ...
        ChAOServiceManager.getInstance().archiveNote(this.project.getProjectName(), this.record.getEntity().getName(), archive, new AsyncCallback<Boolean> (){
    ...
    private void deleteNote(){
        ...
        final String id = this.record.getEntity().getName();
        if(id == null){
    ...
    private void doGetReplies(){
        ChAOServiceManager.getInstance().getReplies(project.getProjectName(), this.record.getEntity().getName() , true,
    ...
    private void edit(){
        ...
        if (!isEditable()){
            MessageBox.alert("Edit is not allowed! You may edit only notes that were created by you and which did not get any replies yet.");
            return;
        }
