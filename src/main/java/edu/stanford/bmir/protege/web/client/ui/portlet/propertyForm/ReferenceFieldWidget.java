...
public class ReferenceFieldWidget extends InstanceGridWidget {
    ...
    private Map<String, Object> bpSearchProperties;
    ...
    public void setup(Map<String, Object> widgetConfiguration, PropertyEntityData propertyEntityData) {
     ...
     protected Anchor createReplaceNewValueHyperlink() {
       Anchor addNewLink = new Anchor("<br><img src=\"images/add.png\"></img>&nbsp;Replace term", true);
    ...
    protected Anchor createReplaceExistingHyperlink() {
        Anchor addNewLink = new Anchor("<br><img src=\"images/add.png\"></img>&nbsp;Find & Replace <br/>term", true);
        addNewLink.addClickHandler(new ClickHandler() {
            public void onClick(ClickEvent event) {
                if (UIUtil.confirmOperationAllowed(getProject())){
                //TODO fix this
                ...
                onAddNewReference(prop == null ? "" : UIUtil.getDisplayText(prop), true);
                }
            }
        });
    ...
    protected Anchor createAddExistingHyperlink() {
        Anchor addNewLink = new Anchor("<br><img src=\"images/add.png\"></img>&nbsp;Find term", true);
        addNewLink.addClickHandler(new ClickHandler() {
            public void onClick(ClickEvent event) {
                //TODO fix this
                //onAddNewReference((String) ReferenceFieldWidget.this.getWidgetConfiguration().get(FormConstants.LABEL));
                PropertyEntityData prop = getProperty();
                onAddNewReference(prop == null ? "" : UIUtil.getDisplayText(prop), false);
            }
        });
    ...
    public Anchor createAddNewValueHyperlink() {
        Anchor addNewLink = new Anchor("<br><img src=\"images/add.png\"></img>&nbsp;Add term", true);
        addNewLink.addClickHandler(new ClickHandler() {
            public void onClick(ClickEvent event) {
                if (UIUtil.confirmOperationAllowed(getProject())) {
                    onCreateNewReference();
    ...
    private void onAddNewReference(String propertyName, boolean replaceExisting) {
        final Window window = new Window();
        window.setTitle("BioPortal Search" + (propertyName == null ? "" : " for property " + propertyName));
        window.setWidth(800);
