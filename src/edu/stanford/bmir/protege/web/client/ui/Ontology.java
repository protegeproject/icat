...
public class Ontology extends TabPanel {
    ...
    public Ontology(Project project, String tabName, String selectionName) {
        super();
    ...
    protected void createOntolgyForm() {
        ...
        doLayout();
        if (tabName != null){
            final String tabClassName = UIFactory.getAvailableTabNameMap().get(tabName);
            final AbstractTab tab = getTabByClassName(tabClassName);
            tab.setSelection(selectionName);
            activate(tab.getId());
        }
    ...
    protected void createToolbarButtons() {
        //TODO: All these should go in a top panel menu
        //TODO: Disable for ICD for now - add later
    ...
    protected ToolbarMenuButton getAddTabButton() {
        ...
        // TODO: this should be moved to another menu in the TopPanel
        Item item2 = new Item("Save configuration");
        addTabMenu.addItem(item2);
        item2.addListener(new BaseItemListenerAdapter() {
            @Override
            public void onClick(BaseItem item, EventObject e) {
