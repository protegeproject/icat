...
public class ParentsPanel extends Panel {
    ...
    private HandlerRegistration removeParentHandlerRegistration;
    private ClickHandler removeParentHandler;
    private String topClass;

    public ParentsPanel(Project project) {
        this(project, true, true, true, true);
        setHeight(70);
    }

    public ParentsPanel(Project project, boolean showAdd, boolean showRemove) {
        this(project, showAdd, showRemove, true, true);
        setHeight(70);
    }

    public ParentsPanel(Project project, boolean showAdd, boolean showRemove, boolean showLabel, boolean showBorder) {
        super();
...
//                    "\" style=\"text-decoration: underline;\">Add parent(s)</a>", "");
    ...
    protected ClickHandler getRemoveParentHandler() {
        if (removeParentHandler == null) {
            removeParentHandler = new ClickHandler() {
                public void onClick(ClickEvent event) {
                    ...
                    if ("a".equalsIgnoreCase(getTagName(target))) {
                        ...
                        if (href.startsWith(UIUtil.LOCAL + UIUtil.REMOVE_PREFIX)) {
                            onRemove(href.substring(UIUtil.REMOVE_PREFIX.length() + UIUtil.LOCAL.length()));
                        }
                    }
                }
            };
        }
        return removeParentHandler;
    }
    ...
    protected HTML getHTML(EntityData parent) {
        ...
        removeParentHandlerRegistration = html.addClickHandler(getRemoveParentHandler());
        return html;
    }
    ...
    protected void onDestroy() {
        ...
        if (removeParentHandlerRegistration != null) {
            removeParentHandlerRegistration.removeHandler();
        }
        removeParentHandler = null;
        super.onDestroy();
    }
    ...
    public Selectable getSelectable() {
        if (selectable == null) {
            ClassTreePortlet selectableTree = new ClassTreePortlet(project, false, false, false, true, topClass);
