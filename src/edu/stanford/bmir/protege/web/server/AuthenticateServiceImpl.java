...
public class AuthenticateServiceImpl extends RemoteServiceServlet implements AuthenticateService {
    ...
    public boolean addFacebookUserDetailsToSession(String provider, String profileUrl, String userId, String loginMethod) {
        try {
            HttpServletRequest request = this.getThreadLocalRequest();
            HttpSession session = request.getSession();
            session.setAttribute(OpenIdConstants.HTTPSESSION_OPENID_ID, userId);
            session.setAttribute(OpenIdConstants.HTTPSESSION_OPENID_PROVIDER, provider);
            session.setAttribute(OpenIdConstants.HTTPSESSION_OPENID_URL, profileUrl);
            session.setAttribute(AuthenticationConstants.LOGIN_METHOD,
                    loginMethod);
        } catch (Exception e) {
            Log.getLogger().log(Level.SEVERE, "Exception in addFacebookUserDetailsToSession", e);
            return false;
        }
        return true;
    }
    ...
    public String getFacebookAPIKey() {
        return ApplicationProperties.getFacebookApiKey();
    }
}