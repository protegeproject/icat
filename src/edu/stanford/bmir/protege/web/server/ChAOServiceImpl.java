...
public class ChAOServiceImpl extends RemoteServiceServlet implements ChAOService {
    ...
    private NotesData getDiscussionThread(Annotation annotation, KnowledgeBase changesKb, boolean topLevelOnly) {
        ...
        try {
            ...
            if (Log.getLogger().getLevel().equals(Level.FINE)) {
                Log.getLogger().fine(
                        "Could not set archived flag on NotesData. Most likely ChAO does not contain a 'archived' slot");
            }
        }
    ...
    public NotesData createNote(String projectName, NotesData newNote, boolean topLevel) {
        ...
        String annotatedEntityName = (newNote.getAnnotatedEntity() == null ? null : newNote.getAnnotatedEntity().getName());
    ...
    public ChangeData createChange(String projectName, String userName, String entityName, final String action,
        ...
        try {
            ...
            log("Caught when trying to append change in project " + projectName + " for user " + userName + ", entity = "
                    + entityName + ", action = " + action + " and context = " + context, e);
            throw e;
        }
    }
    ...
    public PaginationData<ChangeData> getWatchedEntities(String projectName, String userName, int start, int limit, String sort,
            String dir) {
    ...
    private Collection<Ontology_Component> getAllOCsInWatchedBranches(KnowledgeBase kb,
            Collection<Ontology_Component> watchedBranches) {
    ...
    public EntityData addWatchedEntity(String projectName, String userName, String watchedEntityName) {
        ...
        entityData.setWatch(WatchedEntitiesCache.getCache(project).isWatchedBranch(userName, watchedEntityName) ? Watch.BOTH
                : Watch.ENTITY_WATCH);
    ...
    public EntityData addWatchedBranch(String projectName, String userName, String watchedEntityName) {
        ...
        EntityData entityData = OntologyServiceImpl.createEntityData(frame, true);
        entityData.setWatch(WatchedEntitiesCache.getCache(project).isWatchedEntity(userName, watchedEntityName) ? Watch.BOTH
                : Watch.BRANCH_WATCH);
    ...
    /* (non-Javadoc)
     * @see edu.stanford.bmir.protege.web.client.rpc.ChAOService#getChangesByDescription(java.lang.String, java.lang.String, java.lang.String, int, int, java.lang.String, java.lang.String)
     */
    public PaginationData<ChangeData> getChangesByDescription(String projectName, String entityName, String filterString,
            int start, int limit, String sort, String dir) {
        ArrayList<ChangeData> changeDataList = (ArrayList<ChangeData>) getChanges(projectName, entityName);
        ArrayList<ChangeData> filteredChangeDataList = new ArrayList<ChangeData>();
        PaginationData<ChangeData> wrapper = new PaginationData<ChangeData>();

        for (Iterator iterator = changeDataList.iterator(); iterator.hasNext();) {
            ChangeData changeData = (ChangeData) iterator.next();
            if (changeData.getDescription() != null && changeData.getDescription().contains(filterString)) {
                filteredChangeDataList.add(changeData);
            }
        }

        int totalRecords = filteredChangeDataList.size();
        wrapper.setTotalRecords(totalRecords);
        ...
        for (int i = start, k = 0; i <= end; i++, k++) {
            ChangeData record = filteredChangeDataList.get(i);
            wrapper.getData().add(record);
        }
    ...
    /* 
     * Retrieves changes filtered by author string.
     * (non-Javadoc)
     * @see edu.stanford.bmir.protege.web.client.rpc.ChAOService#getChangesByAuthor(java.lang.String, java.lang.String, java.lang.String, int, int, java.lang.String, java.lang.String)
     */
    public PaginationData<ChangeData> getChangesByAuthor(String projectName, String entityName, String filterString, int start,
        ...
        for (Iterator iterator = changeDataList.iterator(); iterator.hasNext();) {
            ...
            if (changeData.getAuthor() != null && changeData.getAuthor().contains(filterString)) {
                filteredChangeDataList.add(changeData);
            }
        }
    ...
    public PaginationData<ChangeData> getChangesByDate(String projectName, String entityName, Date fromDate, Date toDate,
            int start, int limit, String sort, String dir) {
        ...
        for (Iterator iterator = changeDataList.iterator(); iterator.hasNext();) {
            ChangeData changeData = (ChangeData) iterator.next();
            Date changeDataDate = changeData.getTimestamp();
            if(changeDataDate.after(fromDate) && changeDataDate.before(toDate)){
                filteredChangeDataList.add(changeData);
            }
        }
