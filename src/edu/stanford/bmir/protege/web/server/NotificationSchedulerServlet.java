        ...
        private void getAnnotationChanges(String projectName, final Date lastRunDate, Date end, Map<String, Set<NotificationChangeData>> collector,
            ...
            for (Instance annotationInstance : annotationInstances) {
                ...
                for (User user : users) {
                    ...
                    changeData.add(new NotificationChangeData(author.toString(), annotationInstance.getBrowserText(),
                            AnnotationCache.getChangeDate(chaoKb.getProject().getName(), annotationInstance), projectName, rootNode.getComponentType(), className, NotificationType.COMMENT));
                    collector.put(user.getName(), changeData);
                }

                final NotificationChangeData dataWithProject = new NotificationChangeData(author, annotationInstance.getBrowserText(),
                        AnnotationCache.getChangeDate(chaoKb.getProject().getName(), annotationInstance), projectName, rootNode.getComponentType(), className, NotificationType.COMMENT);
